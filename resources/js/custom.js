;(function($, window, document, undefined) {
    ...
    $(document).ready(function() {
        ...
        $('.league-rounds-menu').on('change', function() {
            var round = $(this).val();
            $.ajax({
                url: crb_options.ajax_url,
                type: 'GET',
                data: {
                    action: 'get_league_round_matches',
                    round: round,
                    league_id: 8
                },
                success: function(result) {
                    var matches = $(result).find('.matches-list').html()
                    $('.matches-list').html(matches)
                }
            })
        });
        ...
        $('.league-standings li:not(.table_head)').on('click', function() {
            var link = $(this).find('.team_name a').attr('href');
            document.location = link;
        });


        $('.consecutive-matches-filter').on('change', function() {
            var filterType = 'team';
            var value = $(this).val();
            var table = $('.consecutive-matches table');

            table.find('td,th,tr').removeClass('hidden');
            ...
            if ($(this).hasClass('type-filter')) {
                $('.team-filter').val('all');
                var rowsToHide = table.find('.table-type:not(.' + value + ')');
                rowsToHide.addClass('hidden');
            } else {
                $('.type-filter').val('all');
                var cellsToHide = table.find('.table-team:not(.' + value + ')');
